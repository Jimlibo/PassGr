...
def passes_cost(ar):
    url = 'http://localhost:9103/interoperability/api/PassesCost/' + ar.op1 + '/' + ar.op2 + '/' + str(ar.datefrom) + '/' + str(ar.dateto)  # url with params
    ...
    if ar.format == 'json' & res.status_code == 200:  # if json format was requested and query was successfull
...
parser.add_argument('--op2', help='Operator that owns the station', required=True)
